...
These Hibernate annotations are also very useful to know about:
...
| `@CreationTimestamp`, `@UpdateTimestamp` | Automatically assigns a timestamp to the field
| `@FilterDef`, `@Filter`                  | Defines a Hibernate filter
| `@FetchProfile`                          | Defines a Hibernate fetch profile
| `@GenericGenerator`                      | Selects a custom id generator
| `@DynamicInsert` and `@DynamicUpdate`    | Generates SQL dynamically with only needed columns
                                             (instead of using SQL generated at startup)
| `@Fetch`                                 | Specifies the fetching mode for an association
| `@BatchSize`                             | Specifies the batch size for batch fetching an
                                             association
| `@SqlInsert`, `@SqlUpdate`, `@SqlDelete` | Specifies custom DML for entity operations
|===
...
When using Hibernate Reactive _outside_ of the Quarkus environment, you'll
...
session.find(Author.class, author.id)
       .chain( author -> Stage.fetch(author.books) )
       .invoke( books -> ... )
----
