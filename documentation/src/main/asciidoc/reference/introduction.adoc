...
| PostgreSQL | `io.vertx:vertx-pg-client:{vertx-version}`
| MySQL      | `io.vertx:vertx-mysql-client:{vertx-version}`
| DB2        | `io.vertx:vertx-db2-client:{vertx-version}`
|===

Where `{vertx-version}` is the version of Vert.x compatible with the
...
NOTE: These configuration properties have `jdbc` in their names, but of
course there's no JDBC in Hibernate Reactive, and these are simply the
legacy property names defined by the JPA specification and Hibernate ORM.
In particular, Hibernate Reactive itself parses and interprets the JDBC
URL.
