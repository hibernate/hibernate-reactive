...
public final class ReactiveQueryExecutorLookup {
	...
	public static ReactiveQueryProducer extract(final SharedSessionContractImplementor session) {
		if ( session instanceof org.hibernate.reactive.session.internal.ReactiveSessionImpl ) {
			return (org.hibernate.reactive.session.internal.ReactiveSessionImpl) session;
		}
		else {
