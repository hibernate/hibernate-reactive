...
public class EagerElementCollectionForEmbeddableTypeListTest extends BaseReactiveTest {
	...
	public void updateCollectionWithDuplicatesWithStageAPI(TestContext context) {
		...
		test( context, getSessionFactory().withTransaction( (session, tx) -> session.persist( thomas ) )
				.thenCompose( v -> getSessionFactory().withTransaction( (newSession, tx) -> newSession
						.find( Person.class, thomas.getId() )
	...
	@Test
	@Ignore// Fails on Db2, we have to wait for Vert.x Client 4.1.0: https://github.com/eclipse-vertx/vertx-sql-client/issues/920
	...
	public void deleteElementsFromCollectionWithDuplicatesWithStageAPI(TestContext context) {
		...
		test( context, getSessionFactory()
				.withTransaction( (session, tx) -> session.persist( thomas ) )
				.thenCompose( v -> getSessionFactory().withTransaction( (session, tx) -> session
						.find( Person.class, thomas.getId() )
						// Change one of the element in the collection
						.thenAccept( found -> {
	...
	public void clearCollectionElementsStageAPI(TestContext context) {
		test( context, getSessionFactory()
				...
				.thenCompose( s -> getSessionFactory().withSession( session -> session
						.find( Person.class, thePerson.getId() )
	...
	private Uni<List<Object>> selectFromPhonesWithMutiny(Person person) {
		return getMutinySessionFactory().withSession( session -> session
				.createNativeQuery( "SELECT * FROM Person_phones where Person_id = ?" )
				.setParameter( 1, person.getId() )
				.getResultList() );
	}
