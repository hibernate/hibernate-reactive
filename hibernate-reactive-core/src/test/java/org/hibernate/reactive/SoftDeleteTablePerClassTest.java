...
/**
 * Tests @SoftDelete annotation with TABLE_PER_CLASS inheritance strategy.
 *
 * NOTE: Currently disabled due to a potential issue with @SoftDelete support
 * for TABLE_PER_CLASS inheritance in Hibernate ORM/Reactive.
 * When a soft delete is performed via HQL, the entity is not properly marked
 * as deleted in the database, even when @SoftDelete is applied to both the
 * parent and child classes.
 *
 * This needs further investigation to determine if it's a bug or unsupported feature.
 */
public class SoftDeleteTablePerClassTest extends BaseReactiveTest {
	...
	protected CompletionStage<Void> cleanDb() {
		List<String> tables = List.of( SpellBook.TABLE_NAME, Novel.TABLE_NAME, Book.TABLE_NAME );
		return getSessionFactory()
				.withTransaction( s -> loop( tables, table -> s
						.createNativeQuery( "delete from " + table ).executeUpdate() )
				)
				.thenCompose( CompletionStages::voidFuture );
	}

	@Test
	public void testSoftDeleteWithTablePerClass(VertxTestContext context) {
		test( context, getMutinySessionFactory()
				...
				.call( () -> getMutinySessionFactory().withSession( s -> s
						.createNativeQuery( "select id, title, deleted from " + SpellBook.TABLE_NAME + " order by id" )
						.getResultList()
						.invoke( rows -> {
							assertThat( rows ).hasSize( 2 );
							Object[] firstRow = (Object[]) rows.get( 0 );
							// Check that the first book is marked as deleted
							assertThat( markedAsDeleted( firstRow[2] ) ).isTrue();
						} )
				) )
		);
	}
	...
	@Test
	public void testHQLDeleteWithTablePerClass(VertxTestContext context) {
		test( context, getMutinySessionFactory()
				// Delete using HQL
				.withTransaction( s -> s
	...
	private static boolean markedAsDeleted(Object obj) {
	...
	public static class Book {
		private static final String TABLE_NAME = "SoftDeleteTablePerClassTest_BOOK";
	...
	public static class SpellBook extends Book {
		private static final String TABLE_NAME = "SoftDeleteTablePerClassTest_SPELL_BOOK";
	...
	public static class Novel extends Book {
		private static final String TABLE_NAME = "SoftDeleteTablePerClassTest_NOVEL";
