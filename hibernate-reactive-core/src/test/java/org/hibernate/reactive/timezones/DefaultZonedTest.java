						...
						.thenCompose( s -> s.find( Zoned.class, zid )
								.thenAccept( z-> {
									...
									if ( getDialect().getTimeZoneSupport() == TimeZoneSupport.NATIVE ) {
										context.assertEquals( nowZoned.toOffsetDateTime().getOffset(), z.zonedDateTime.toOffsetDateTime().getOffset() );
										context.assertEquals( nowOffset.getOffset(), z.offsetDateTime.getOffset() );
									}
									else {
										context.assertEquals( ZoneId.of("Z"), z.zonedDateTime.getZone() );
										context.assertEquals( ZoneOffset.ofHours(0), z.offsetDateTime.getOffset() );
									}
								} )
						)
				)
