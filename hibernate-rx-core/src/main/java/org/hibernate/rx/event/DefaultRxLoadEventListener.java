    ...
    private CompletionStage<Object> proxyOrLoad(
        ...
        if ( options.isAllowProxyCreation()
            ...
            if ( persister.hasProxy() ) {
                ...
                if ( proxy != null ) {
                    return RxUtil.completedFuture(returnNarrowedProxy( event, persister, keyToLoad, options, persistenceContext, proxy ));
                }

                if ( options.isAllowProxyCreation() ) {
                    return RxUtil.completedFuture(createProxyIfNecessary( event, persister, keyToLoad, options, persistenceContext ));
                }
            }
        }
