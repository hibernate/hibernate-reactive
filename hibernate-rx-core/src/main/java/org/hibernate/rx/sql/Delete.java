...
/**
 * An SQL <tt>DELETE</tt> statement for the reactive PG client
 */
public class Delete {
	...
	public String toStatementString() {
		...
		if ( versionColumnName != null ) {
			...
			buf.append( versionColumnName ).append( "=$" ).append( primaryKeyColumns.size() + 1 );
		}
		return buf.toString();
	}
	...
	public Delete addPrimaryKeyColumns(String[] columnNames) {
		for ( String columnName : columnNames ) {
			addPrimaryKeyColumn( columnName, "$" + (primaryKeyColumns.size() + 1) );
		}
		return this;
	}
